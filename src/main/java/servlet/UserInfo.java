...
public class UserInfo extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		for (Person p : resultList) {
			out.println("<UL>\n");
			out.println("<LI>Nom: " + p.getNom() + "\n");
			out.println("<LI>Prenom: " + p.getPrenom() + "\n");
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		person.setNom(request.getParameter("name"));
		person.setPrenom(request.getParameter("firstname"));
		person.setMail(request.getParameter("mail"));

		Home h1 = new Home();
		h1.setNbrePiece(4);
		h1.setTailleMaison(30);
		h1.setPerson(person);
